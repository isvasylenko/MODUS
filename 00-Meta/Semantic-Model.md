...
Зв'язки контуру змін: `Ціль → породжує → Ініціатива → групується в → Програма → декомпозується у → Проєкт → Епік → Таска`; `Проєкт → розвиває → Capability`; `Програма → реалізує → Вектор`.
